...
1. **스트림 프로세싱** (Stream Processing)
    1. **이벤트 스트림** (데이터 스트림 또는 스틑리밍 데이터)
        - 무한 데이터 세트 (unbounded dataset)를 나타내는 추상 개념
        - 여기서 무한은 끝이 없이 계속 늘어난다는 의미다. 즉, 끊임없이 새로운 데이터가 전달되므로 데이터 세트가 무한이 되는 것이다.
        - 다른 속성
            1. **이벤트 스트림에는 순서가 있다.** : 본질적으로 이벤트는 다른 이벤트 전이나 후에 발생한다. 이것이 이벤트 스트림과 관계형 데이터베이스 테이블의 차이점 중 하나다. 관계형 데이터베이스 테이블의 레코드는 항상 순서가 없는 것으로 간주한다.
            2. **불변 데이터 레코드** : 이벤트 자체는 일단 발생하면 변경될 수 없다. 테이블의 레코드를 삭제하거나 변경할 수 있지만 그것은 데이터베이스 내부에서 처리되는 트랜잭션이며 테이블 자체에 수록되지는 않는다. 반면에, 이벤트 스트림에는 스트림 자체에 모든 트랜잭션이 수록된다.
            3. **이벤트 스트림은 재생 가능할 수 있다.** : 카프카는 이벤트 스트림을 캡처하고 재생할 수 있다.
        - 이벤트는 시스템에 따라 다르므로, 작을 수도 있고 매우 클 수도 있다.
        - 이벤트는 내부 구조가 없거나, 키-값의 쌍으로 된 구조이거나, JSON 형식이거나, 또는 나름의 구조를 갖는 Avro나 Protobuf 메시지일 수도 있다.
    2. **스트림 프로세싱은 하나 이상의 이벤트 스트림을 처리하는 것을 나타낸다.** 스트림 프로세싱은 요청-응답과 배치 프로세싱처럼 프로그래밍 패러다임이다.
        1. **요청-응답** (request-response) : 가장 지연이 적은 패러다임이다. 즉, 1밀리초 이하에서 수밀리초까지의 짧은 응답 시간을 가지며, 응답 시간이 크게 달라지지 않는다. 그러나 애플리케이션이 요청을 전송한 후 프로세싱 시스템의 응답을 기다려야 한다.
        2. **배치 프로세싱** (batch processing) : 지연이 많지만 처리량이 크다. 이 프로세싱 시스템은 설정된 시간에 시작된다. 작업 처리 시간은 수분부터 수시간까지이며, 사용자들은 시간이 지난 데이터라는 것을 염두에 두고 결과를 살펴본다. 이 패러다임은 효율성이 좋고 규모의 경제를 구현한다. 그러나 더 시기적절하고 효율적인 결정을 하기 위해, 근래에는 단기간에 사용할 수 있는 데이터 비즈니스에서 요구한다.
        3. **스트림 프로세싱** : 연속적이며 중단되지 않는 패러다임이며, 요청-응답과 배치 프로세싱 간의 격차를 줄여준다. 대부분의 비즈니스 프로세스는 수밀리초 이내의 즉시 응답을 요구하지 않지만, 그렇다고 다음 날까지 기다려주지는 않는다. 또한 대부분의 비즈니스 프로세스는 연속적으로 발생한다. 그리고 비즈니스 리포트가 지속적으로 변경되고 이와 관련된 비즈니스 애플리케이션들 역시 계속해서 응답할 수만 있다면, 수밀리초 이내의 빠른 응답이 아니더라도 프로세싱을 진행할 수 있다.
    3. **무한 데이터세트로부터 계속해서 데이터를 읽고, 처리하고, 출력을 내보낸다면 그 자체로 스트림 프로세싱을 하고 있는 것이다. 단, 지속적으로 계속 진행되는 것이어야 한다.**
2. **스트림 프로세싱 개념**
    1. **시간** (time)
        - **시간은 스트림 프로세싱의 가장 중요한 개념**
        - **스트림 프로세싱의 관점에서는 시간의 공통적인 개념이 중요**하다. 대부분의 스트림 프로세싱 애플리케이션들이 **타임 윈도우**(time window, 고정된 시간 간격) 작업으로 수행되기 때문이다.
        - 시간 개념
            1. **이벤트 시간** (event time) : **우리가 추적하는 이벤틑가 발생하여 레코드가 생성된 시간**이다. 0.10.0 이상 버전부터 카프카는 자동으로 프로듀서 레코드에 생성 시간을 추가한다. 따라서 만일 이 시간이 우리 애플리케이션의 이벤트 시간 개념과 일치하지 않는다면, 이벤트 시간을 레코드 자체의 필드로 추가해야 한다. 이벤트 시간은 스트림 데이터를 처리할 때 가장 중요한 시간이다.
            2. 로그 추가 시간 (log append time) : **이벤트가 카프카 브로커에 전송되어 저장된 시간**이다. 0.10.0 이상 버전부터 카프카 브로커는 자신이 수신한 레코드에 이 시간을 자동으로 추가한다. 단, 그렇게 하도록 카프카가 구성되었거나 타임스탬프가 없는 레코드를 구버전의 프로듀서가 쓰는 경우다. 스트림 프로세싱에서는 이벤트가 발생된 시간이 관심사이므로 이런 시간 개념이 덜 중요하다. 그러나 이벤틑가 실제로 발생된 시간이 수록되지 않는 경우에는 여전히 로그 추가 시간을 사용할 수 있다.
            3. 처리 시간 (processing time) : **스트림 프로세싱 애플리케이션이 처리를 수행하기 위해 이벤트를 받은 시간**이다. 이 시간 개념은 신뢰성이 매우 낮으므로 사용하지 않는 것이 좋다.
        - 시간을 사용할 때는 시간대(time zone)를 염두에 두는 것이 중요하다. 데이터 파이프라인 전체에서 단일 시간대로 통일해서 사용해야 한다. 그렇지 않으면 스트림 작업에 혼란이 생겨서 데이터 자체가 무의미해질 수 있기 때문이다.
    2. **상태**
        - 각 이벤트를 개별적으로만 처리한다면 스트림 프로세싱은 매우 간단하다. 그러나 다수의 이벤트가 포함되는 작업을 갖는 스트림 프로세싱은 복잡해진다. 이런 경우 각 이벤트 자체만 살펴보는 것으로는 충분하지 않으므로 더 많은 정보를 추적 관리해야 한다. 예를 들어, 현재의 한 시간 동안 발생한 각 타입의 이벤트 수, 조인과 합계 및 평균을 구해야 하는 모든 이벤트 등이다. 이처럼 이벤트 간에 저장되는 정보를 상태(state)라고 한다.
        - 상태 유형
            1. **로컬 또는 내부 상태** (local or internal state)
                - 스트림 프로세싱 애플리케이션의 특정 인스턴스에서만 사용할 수 있는 상태다. 이 상태는 애플리케이션 내부에 포함되어 메모리에서 실행되는 데이터베이스를 사용해서 유지 관리된다. 따라서 처리 속도가 매우 빠르다는 것이 장점이다. 반면에 사용할 수 있는 메모리의 제한을 받는다는 단점이 있다.
            2. **외부 상태** (external state)
                - 카산드라(Cassandra)와 같은 NoSQL 외부 데이터스토어나 이외의 다른 시스템에 저장되고 유지 관리되는 상태다. 크기 제한이 거의 없고, 스트림 프로세싱 애플리케이션의 여러 인스턴스 또는 다른 애플리케이션에서도 사용할 수 있다는 것이 외부 상태의 장점이다. 반면에 다른 시스템이 추가되는 데 따른 지연과 복잡도가 생기는 것이 단점이다.
    3. **타임 윈도우**
        - 대부분의 스트림 작업은 타임 윈도우 작업으로 수행된다. 예를 들어, 이동 평균 산출, 이번 주에 제일 많이 판매된 제품 찾기 등이다.
        - 두 스트림의 조인도 타임 윈도우 작업으로 수행된다. 즉, 같은 시점의 같은 시간 간격에 발생된 이벤트들을 조인한다.
        - 타임 윈도우 작업에서 알아야 할 사항
            1. 타임 윈도우의 크기 : 예를 들어, 모든 이벤트의 이동 평균을 매 5분 또는 15분 또는 하루 중의 어떤 타임 윈도우로 산출할 것인지를 나타낸다. 타임 윈도우가 클수록 지연도 커진다.
            2. 타임 윈도우의 이동 빈도 : 예를 들어, 5분의 타임 윈도우로 산출된 이동 평균을 매분, 매초, 또는 새로운 이벤트가 생길 때마다 중에서 언제 변경할 것인가다. 타임 윈도우가 스트림을 따라 이동하는 진행 간격(advance interval) 시간이 타임 윈도우 크기와 같은 경우를 텀블링 윈도우(tumbling window)라고 하며, 다른 경우를 호핑 윈도우(hopping window)라고 한다. 또한 타임 윈도우가 모든 레코드를 따라서 이동하는 것을 슬라이딩 윈도우(sliding window)라고 한다.
            3. 타임 윈도우의 이벤트 변경 가능 시간 : 예를 들어, 00:00-00:05 타임 윈도우의 이동 평균이 산출되었다고 해보자. 그리고 한 시간이 지난 후에 00:02의 이벤트 시간에 추가될 결괏값을 얻게 되었다. 이런 경우 이 결괏값을 00:00-00:05 타임 윈도우의 결괏값에 변경해야 할까? 아니면 지난 시간의 결괏값은 그대로 내버려 두어야 할까? 이상적으로는 이벤트가 추가되는 특정 시간대의 타임 윈도우를 정의할 수 있다. 예를 들어, 이벤트를 4시간 늦게까지 받는다면 그것으로 산출되는 결괏값을 다시 계산하여 변경하고, 4시간 이후의 이벤트들은 무시할 수 있다.
3. **카프카 스트림즈 사용 예**
    1. **카프카 스트림즈**
        - 아파치 카프카는 두 개의 스트림 API를 갖고 있다. 저수준의 프로세서 API와 고수준의 스트림 DSL(domain specific language)이다.
        - 카프카 스트림즈 DSL API를 사용하는 애플리케이션은 항상 StreamBuilder 클래스를 사용해서 프로세싱 토폴로지(topology)를 생성하는 것으로 시작한다. (토폴로지는 스트림의 이벤트에 적용되는 변환의 DAG(directed acyclic graph, 방향성이 있는 비순환성 그래프)이다.)
        - 그 다음에 해당 토폴로지로부터 KafkaStreams 실행 객체를 생성한다. 그리고 KafkaStreams 실행 객체를 시작시키면 다수의 스레드가 시작되며 각 스레드는 해당 프로세싱 토폴로지를 스트림의 이벤트에 적용한다.
        - 그리고 KafkaStreams 실행 객체를 닫으면 프로세싱이 끝난다.
    1. **단어 개수 세기** [코드](https://github.com/gwenshap/kafka-streams-wordcount)
        - 맵/필터 패턴과 간단한 집계 스트림
        - **실행을 위해 필요한 코드 수정**
            1. **WordCountExample에 BOOTSTRAP_SERVERS_CONFIG 변경**
            2. **pom.xml에 dependency 수정**
                <p align="center">
                    <img src="Images/stream1.png",width="100%">
                </p>
        - **집계 결과**
            <p align="center">
                <img src="Images/stream2.png",width="100%">
            </p>
    2. **주식 시장 거래 통계** [코드](https://github.com/gwenshap/kafka-streams-stockstats)
        - 타임 윈도우 집계 스트림
        - 실행을 위해 필요한 코드 수정
            1. **Constants에 BROKER 주소 수정**
            2. **StockGenProducer와 StockStatsExample의 Properties 객체 선언** (별도의 구성 파일을 생성하지 않고 빠른 실행, 이경우 LoadConfigs 사용X)
                <p align="center">
                    <img src="Images/stream5.png",width="100%">
                </p>
        - **생성된 데이터** (StockGenProducer)
            <p align="center">
                <img src="Images/stream3.png",width="100%">
            </p>
        - **집계 결과** (StockStatsExample)
            <p align="center">
                <img src="Images/stream4.png",width="100%">
            </p>
